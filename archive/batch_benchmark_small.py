...
def load_trajectories(num_trajectories=10):
    """从jsonl文件加载轨迹数据"""
    trajectories = []
...
def construct_messages(trajectory, system_prompt, instance_prompt_template):
    ...
    # 处理trajectory steps（只取前3步作为示例）
    steps = trajectory.get('trajectory_steps', [])[:3]
    for step in steps:
        ...
        observation = step.get('observation', '')[:500]  # 截断observation
...
def benchmark_batch(runtime, trajectories, batch_size, config):
    ...
    for i in range(0, len(all_messages), batch_size):
        ...
        for messages in batch_messages:
            try:
                state = process_single_trajectory.run(messages=messages)
                processed += 1
                # 统计生成的token数
        ...
        print(f"  批次 {i//batch_size + 1}: 处理了 {len(batch_messages)} 条, "
...
def main():
    ...
    # 保存结果
    output_file = "benchmark_results_small.json"
